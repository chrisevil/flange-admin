...
export function HorizontalBarChartCard<T extends Record<string, any>>({
    ...
    setDrilldownState((prev) => {
      ...
      return {
        ...
        currentTitle:
          prev.level === 1
            ? title
            : `${String(newParentItems[newParentItems.length - 1][labelKey])} 详情`,
      };
    });
  }, [data, title, labelKey]);
  ...
  const displayData = maxItems
    ? drilldownState.data.slice(0, maxItems)
    : drilldownState.data;
  ...
  return (
    <Card className={cn("overflow-hidden", className)}>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div>
              ...
              {description && drilldownState.level === 0 && (
      ...
      <CardContent>
        <div
          className={cn("w-full", !height && "h-[350px]")}
                ...
                {segments ? (
                  segments.map((seg, index) => (
                    <Bar
                      ...
                      cursor={
                        drilldownKey && getDrilldownData ? "pointer" : undefined
                      }
                    />
                  ))
                ) : (
