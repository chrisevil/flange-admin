...
import type { processDepartmentData } from "../_libs/data-fetcher"; // Import the return type

// Use the type from data-fetcher
type DepartmentData = ReturnType<typeof processDepartmentData>[number];
type EmployeeDetail = DepartmentData["employeesByLevel"][keyof DepartmentData["employeesByLevel"]][number];

// Remove the old interface definitions
...
interface DepartmentTabProps {
  // Update the prop type to use the imported type
...
//     const employeeCount =
//       dept.employeeCount || Math.floor(Math.random() * 50) + 10; // 模拟员工数
//     const avgDailyOvertimeHours = dept.avgHours
//       ? dept.avgHours / 22
//       : Math.random() * 3; // 模拟人均日加班
//     const lastMonthAvgHours =
//       avgDailyOvertimeHours * (Math.random() * 0.4 + 0.8); // 模拟上月数据
...
export function DepartmentTab({ departmentDataQuery }: DepartmentTabProps) {
  ...
  // Directly use and sort the data from props, remove mock data call
  const processedData = React.useMemo(
    () =>
      (departmentDataQuery.data ?? []).sort(
        (a, b) => b.avgDailyOvertimeHours - a.avgDailyOvertimeHours,
      ), // 按人均日加班降序
    ...
    <div className="space-y-6 pt-4">
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-1">
          <RankList
            ...
            itemRender={(item, idx, isAboveAvg, avg) => {
              // Remove comparison logic as lastMonthAvgHours is not available
              // const change = item.lastMonthAvgHours
              //   ? item.avgDailyOvertimeHours - item.lastMonthAvgHours
              //   : 0;
              // const ChangeIcon =
              //   change > 0 ? ArrowUp : change < 0 ? ArrowDown : ArrowRight;
              // const iconColor =
              //   change > 0
              //     ? "text-red-500"
              //     : change < 0
              ...
              return (
                <div
                  key={idx}
                  className="flex cursor-pointer items-center py-2 hover:bg-muted/50"
                  onClick={() => handleRankItemClick(item)}
                >
                  ...
                  {/* Remove change icon display */}
                  {/* {item.lastMonthAvgHours !== undefined && (
                    <span
                      className={`ml-2 flex items-center text-xs ${iconColor}`}
                    >
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:col-span-2">
          {processedData.map((dept, idx) => {
            ...
            const levelEmployeeData = (["A", "B", "C", "D"] as const)
              .map((level) => ({
                level: level,
              ...
              <Card key={`${dept.department}-${idx}-info`}>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  ...
                  <div className="flex items-center space-x-2">
                    <Button
                      ...
                      onClick={() => handleRankItemClick(dept)}
                    >
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-muted-foreground">
                    人均日加班: {dept.avgDailyOvertimeHours.toFixed(1)} 小时/日
                  </p>

                  {/* 横向堆叠柱状图 */}
                  <div className="mt-2 h-[180px] w-full p-0">
                    <HorizontalBarChartCard
                      height={150}
                      data={chartData}
                      labelKey="distributionLabel"
                      segments={segments}
                      className="h-full border-none p-0 shadow-none relative left-[-20px]"
                    />
                  </div>

                  {/* 等级员工排行榜 */}
                  <div className="mt-2 text-xs">
                    {levelEmployeeData.map(
                      (levelData) =>
                        levelData.count > 0 && (
                          <div
                            key={levelData.level}
                            ...
                            {levelData.employees.length > 0 && (
                              <span className="ml-2 flex-1 truncate text-muted-foreground">
                                {levelData.employees
                                  ...
                                  .map((emp) => emp.name)
                                  .join(", ")}
                                {levelData.employees.length > 3
                                  ? ` 等${levelData.employees.length}人`
                                  : ""}
                              </span>
                            )}
                          </div>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            <DialogTitle>
              {selectedDepartment?.department} - 加班等级详情
            </DialogTitle>
            <DialogDescription>各加班等级下的员工列表。</DialogDescription>
          </DialogHeader>
          {selectedDepartment && (
            <div className="mt-4 grid max-h-[60vh] grid-cols-1 gap-4 overflow-y-auto md:grid-cols-2">
              {(['A', 'B', 'C', 'D'] as const).map((level) => (
                <div key={level}>
                  ...
                  >
                    {level}级 (
                    {selectedDepartment.employeesByLevel[level].length}人)
                  </h3>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          {/* Optionally add daily average if needed */}
                          {/* <TableHead className="text-right">日均加班</TableHead> */}
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {selectedDepartment.employeesByLevel[level].length >
                        0 ? (
                          selectedDepartment.employeesByLevel[level].map(
                            (emp) => (
                              <TableRow key={emp.id}>
                                <TableCell>{emp.name}</TableCell>
                                <TableCell className="text-right">
                                  {/* Use overtimeHours from the actual data */}
                                  {emp.overtimeHours.toFixed(1)}
                                </TableCell>
                                {/* Optionally display daily average */}
                                {/* <TableCell className="text-right">
                                  {emp.dailyAvgHours.toFixed(1)}
                                </TableCell> */}
                              </TableRow>
                            ),
                          )
                          ...
                          <TableRow>
                            <TableCell
                              colSpan={2} // Adjust colspan if adding columns
